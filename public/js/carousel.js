...
async function createCarousel() {
    let data = await fetch("https://udemy-backend-8qgk.onrender.com/coursesData");
...
async function addToWishlist(p) {
    ...
    if (cookie.length > 1) {
        ...
        let out = await fetch(`https://udemy-backend-8qgk.onrender.com/login/updateWishlist/${userId}`, {
            method: "PATCH",
            headers: { "Content-type": "application/json" },
            body: JSON.stringify({
                product: p
